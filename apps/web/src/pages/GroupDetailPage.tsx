    ...
    <article className="space-y-8">
      <Button variant="ghost" size="sm" asChild className="-ml-3">
        <Link to={`/groups/${slug}`}>
          ...
          Back to {group.title} roadmap
        </Link>
      </Button>

      <header className="space-y-3">
        <Badge variant="accent" className="rounded">Group {group.order}</Badge>
        ...
        {group.hasTest ? (
          <Card>
            <CardContent className="flex flex-col items-start gap-3 pt-6 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="font-medium">{group.testExerciseCount} contrast questions</p>
                <p className="text-sm text-muted-foreground">
                  Mixed-tense questions to lock in the differences between the modules in this group.
                </p>
              </div>
              <Button asChild>
          ...
          <Card>
            <CardContent className="pt-6 text-sm text-muted-foreground">
              The group test for <strong>{group.title}</strong> hasn't been authored yet.
            </CardContent>
          </Card>
        )}
      </section>
